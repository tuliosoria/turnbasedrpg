...
import { countWikiEntries, listWikiEntries } from "../db/wiki";
...
export async function getWikiCount(deps: Deps, _req: HandlerRequest): Promise<HandlerResponse> {
  const count = await countWikiEntries(deps.doc, deps.config.tableName, deps.config.campaignId);
